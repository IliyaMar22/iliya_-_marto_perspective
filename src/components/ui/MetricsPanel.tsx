...
export default function MetricsPanel({ region, onClose }: MetricsPanelProps) {
  ...
  return (
    <motion.div
        ...
        className="fixed right-2 sm:right-4 md:right-8 top-1/2 -translate-y-1/2 bg-black/90 backdrop-blur-lg border border-lime-400 rounded-lg shadow-2xl w-[calc(100vw-1rem)] sm:w-[calc(100vw-2rem)] md:w-auto md:max-w-md max-h-[75vh] z-50 flex flex-col"
      >
        {/* Fixed Header */}
        <div className="relative p-4 sm:p-6 md:p-8 pb-4 border-b border-gray-700 flex-shrink-0">
          <button
            onClick={onClose}
            className="absolute top-2 right-2 sm:top-4 sm:right-4 text-gray-400 hover:text-lime-400 active:text-lime-300 transition-colors z-10 touch-manipulation"
          ...
          <h2 className="text-xl sm:text-2xl md:text-3xl font-bold text-lime-400 pr-8 sm:pr-10">{region.name}</h2>
        </div>

        {/* Scrollable Content */}
        <div 
          className="overflow-y-auto overflow-x-hidden flex-1 p-4 sm:p-6 md:p-8 pt-4 pr-4 sm:pr-6 min-h-0 metrics-panel-scrollbar touch-pan-y" 
          style={{ 
            scrollbarWidth: 'thin', 
            scrollbarColor: 'rgba(210, 255, 0, 0.4) rgba(31, 41, 55, 0.8)'
          }}
        >
